...
**1. Save Page Now cannot capture non-HTML over .onion — for anyone.** This was chased for days as a defect of our site. It is not. A four-URL submission in one window returned: our HTML `error:gateway-timeout`, our CSS `error:no-captures`, **DuckDuckGo's onion HTML SUCCESS in 20s**, DuckDuckGo's onion CSS `error:no-captures`. DDG's onion is fast and healthy and captured its HTML on the spot in that same window, so this is neither Tor congestion nor our origin. The discriminator is extension/Content-Type, not size or reachability: 29-byte byte-identical files on our onion capture as `.html` and fail as `.css`. The Wayback Machine *does* hold onion CSS/JS (DDG 82 text/css + 230 JS, IA's own onion 225 JS, dated 202602–202606) but those did not arrive via SPN — CDX redacts the WARC `filename` field so the pipeline can't be read off the index. Treat bare captures over onion as SPN's steady state. Full detail and the caveat about page-*embed* capture (a separate, still-unestablished mechanism) is in the comment above `onionpress_wayback_submit_parallel()`.
